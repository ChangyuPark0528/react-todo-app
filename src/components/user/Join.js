...
const Join = () => {
  ...
  // 각각의 입력창마다 검증 상태를 관리 해야 하기 때문에 객체로 선언
  // 상태를 유지하려는 이유 -> 마지막에 회원 가입 버튼을 누를 때 각각의 input 의 검증을
  ...
  const nameHandler = (e) => {
    ...
    saveInputState({
      key: 'userName',
      inputValue, // inputValue: 'inputValue' 변수이름과 프로퍼티 이름이 같아 생략
      msg,
      flag,
    });
  };
